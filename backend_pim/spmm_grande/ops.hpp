...
void spmm_csr_sparse_to_device_group(struct csr_info_group *sp_group, struct dpu_devices_t *dpu_devices){
	...
	for (uint32_t i = 0; i < sp_group->n_parts; i++) {
		...
		sp_group->sp_info[i]->dpu_info = (struct dpu_info_t_csr *) malloc(info->dense_parts * sizeof(struct dpu_info_t_csr));
		sp_group->sp_info[i]->input_args = (dpu_arguments_t_csr *) malloc(info->dense_parts * sizeof(dpu_arguments_t_csr));
		sp_group->sp_info[i]->max_rows_per_dpu = 0;
		sp_group->sp_info[i]->max_nnz_ind_per_dpu = 0;
		sp_group->sp_info[i]->max_nnz_val_per_dpu = 0;
		sp_group->sp_info[i]->max_rows_per_tasklet = 0;
		uint32_t *dense_ncols = sp_group->dense_ncols[i];
		uint32_t ncols_per_dpu_pad = (sp_group->dense_ncols[i][0] + lcm_ncols - 1) / lcm_ncols * lcm_ncols;
		// printf("%d %d %d\n", ncols_per_dpu_pad, sp_group->dense_ncols[i][0], lcm_ncols);
		sp_group->max_h_size = (sp_group->max_h_size >= dense_ncols[0]) ? sp_group->max_h_size: dense_ncols[0];
		prepare_pim_csr(info, dense_ncols, i, ncols_per_dpu_pad);
		// printf("max_rows_per_dpu: %d\n", info->max_rows_per_dpu);
	}
	tot_time += omp_get_wtime();
	stop(&timer, 0);

	printf("max_rows_per_dpu: ");
	sp_group->max_rows_per_dpu_all_groups = 0;  // TODO: FIXME we probably need the max cols per dpu across all ranks
	for (uint32_t i = 0; i < sp_group->n_parts; i++) {
		struct csr_info *info = sp_group->sp_info[i];
		if (info->max_rows_per_dpu > sp_group->max_rows_per_dpu_all_groups) {
			sp_group->max_rows_per_dpu_all_groups = info->max_rows_per_dpu;
		}
	}
...
//	copy_sparse_csr(sp_group, sp_group->dense_parts);
}
//
///**
// * @brief compute output in the host
...
//			// printf("prepare_pim_coo: %d %d \n", i, j);
//			uint32_t h_size = sp_group->dense_ncols[j];
//			prepare_pim_coo(info, h_size, i, j, sp_group->dense_parts);
//
//		}
//	}
//
