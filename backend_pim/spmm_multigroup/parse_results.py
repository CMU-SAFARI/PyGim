...
def get_data(out_dir, csv_dir):
    ...
    file_name_example = list(avs_results.keys())[0]
    if len(file_name_example.split("_")[1:]) == 6:
        all_labels = ["dtype", "balance", "dataset", "hidden_size", "sp_parts", "ds_parts"] + keys
    elif len(file_name_example.split("_")[1:]) == 9:
        all_labels = ["sp_format", "dtype", "balance", "balance_tsklt", "nr_tasklets", "dataset", "hidden_size", "sp_parts", "ds_parts"] + keys
    elif len(file_name_example.split("_")[1:]) == 10:
        all_labels = ["sp_format", "dtype", "balance", "balance_tsklt", "cg_lock", "nr_tasklets", "dataset", "hidden_size", "sp_parts", "ds_parts"] + keys
    elif len(file_name_example.split("_")[1:]) == 11:
        all_labels = ["sp_format", "dtype", "balance", "balance_tsklt", "merge", "cg_lock", "nr_tasklets", "dataset", "hidden_size", "sp_parts", "ds_parts"] + keys
...
def main():
    ...
    parser.add_argument('--out_path', type=str, default="./results_231003_171530_sync")
